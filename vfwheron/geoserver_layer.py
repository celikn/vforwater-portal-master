...
def test_geoserver_env(store: str, workspace: str):
    ...
    def __build_store():
        ...
        datastore_xml = '<dataStore>' \
                        ...
                        '</dataStore>'.format(store="GetirStore", host=DATABASES['default']['HOST'],
                                              port="8090",
                                              dbname="getirdb",
                                              user="postgres",
                                              passwd="docker")
                        ...
                        # .format(store=store, host=DATABASES['default']['HOST'],
                        #                       port=DATABASES['default']['PORT'],
                        #                       dbname=DATABASES['default']['NAME'],
                        #                       user=DATABASES['default']['USER'],
                        #                       passwd=DATABASES['default']['PASSWORD'])
        rest_url = '{}/rest/workspaces/{}/datastores'.format(LOCAL_GEOSERVER, "Getir")
    ...
    url = '{}/rest/workspaces/{}'.format(LOCAL_GEOSERVER, "Getir")
    ...
    if check.status_code != 200:  # if workspace doesn't exist build it
        ...
        url = '{}/rest/workspaces/{}/datastores/{}'.format(LOCAL_GEOSERVER, "Getir", "GetirStore")
        print(url)
        check = requests.get(url, auth=eval(SECRET_GEOSERVER), headers={"Accept": "application/xml"})
                ...
                '<srs>EPSG:' + str(srid) + '</srs><projectionPolicy>FORCE_DECLARED</projectionPolicy>' \
                                           ...
                                           '<virtualTable><name>' + filename + '</name>' \
                                                                               '<sql>' + query + '</sql>' \
                                                                                                 ...
                                                                                                 '<geometry><name>Geometry</name><type>Point</type><srid>' + str(
        srid) + '</srid></geometry>' \
                '</virtualTable>' \
                '</entry>' \
                '</metadata>' \
